...
lines_start=zeros(length(L),1);%记录经过目标起始站的线路
lines_final=zeros(length(L),1);%记录经过目标终点站的线路
lines_way2=zeros(length(L),1);
lines=[];
for i=1:length(B)
                      ...
                      if(B(jlu_f(line_way),j1)>0) 
                          ...
                          if(~isempty([x,y])&& ~isempty([x1,y1]))
                             ...
                             f=(L{jlu_s(i),2}==1)+(L{jlu_s(i),2}==2)*(((ceil(m/20)>2)*3+(ceil(m/20)<=2)*ceil(m/20)...
                                 (L{jlu_f(i),2}==1)+(L{jlu_f(i),2}==2)*(((ceil(n/20)>2)*3+(ceil(n/20)<=2)*ceil(n/20);
